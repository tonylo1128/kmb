...
export function editting(
  ...
  if (
    (inputA != target.開始時間 && inputB != target.結束時間) ||
    inputD != target.Instagram記錄連結 ||
    inputE != target.備註
  ) {
    ...
    if (inputA != target.開始時間 && inputB != target.結束時間) {
      ...
      startTime =
        inputA.split("T")[0].split("-").join("/") +
        " " +
        inputA.split("T")[1] +
        ":00";
      console.log(inputA);
      endTime =
        inputB.split("T")[0].split("-").join("/") +
        " " +
        inputB.split("T")[1] +
        ":00";
      console.log(endTime);
    ...
    return (dispatch) => {
      ...
      return axios
        .put(url, { startTime, endTime, total, igLink, remake, target })
        .then((resp) => {
...
export function lightModee() {
  console.log("Try Dispatch !!!!!!!");
  return {
    type: cssType.HANDLE_LIGHT_MODE_SWITCH,
  };
}
